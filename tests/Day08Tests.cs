...
namespace tests
{
    public class Day08Tests
    {
        ...
        {
            var sut = new Day08(example_input_1.ReplaceLineEndings("\n").Split('\n'));
        ...
        public void InitialVisibilityShowsAllEdgesVisible()
        {
            ...
            Day08.MarkExtemitiesVisible(visibility);

            visibility.First().Should().AllBeEquivalentTo(Day08.Visibility.Visible);
            visibility.Last().Should().AllBeEquivalentTo(Day08.Visibility.Visible);
            
            for (var i = 1; i < visibility.Length - 2; i++)
            {
                visibility[i][0].Should().Be(Day08.Visibility.Visible);
                visibility[i][1].Should().Be(Day08.Visibility.NotSure);
                visibility[i][2].Should().Be(Day08.Visibility.NotSure);
                visibility[i][3].Should().Be(Day08.Visibility.NotSure);
                visibility[i][4].Should().Be(Day08.Visibility.Visible);
            }
        }
        ...
        {
            var sut = new Day08(interestingPyramid.Split('\n'));
            var vis = sut.GetEmptyVisibilityMatrix();
            sut.MarkVisibleFromWest(vis);
            Day08.FormatVisibility(vis).Should().Be("""
                00000
                01110
                01100
                01000
        ...
        public void Part1WithActualInputProducesCorrectResult()
        {
            ...
            var sut = new Day08(input.ReplaceLineEndings("\n").Split('\n'));
